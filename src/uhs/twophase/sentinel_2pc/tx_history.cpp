...
tx_history_archiver::tx_history_archiver(uint32_t sentinel_id,
                  ...
                  const string& db_param) : m_sentinel_id(sentinel_id), m_logger(logger), m_db_param(db_param) {

    if(!m_sentinel_id) {
        m_logger->info("TX archive is NOT initialized");
        return;
    }

    m_logger->info("Create TxHistory DB in folder", db_param.c_str());
    m_db = DBHandler::createDBHandler(string("leveldb"), db_param, logger);
}
...
auto tx_history_archiver::init(uint32_t sentinel_id, const std::string& db_param) -> bool {
    if(!m_sentinel_id) {
        m_logger->info("Create TxHistory DB in folder", m_db_param.c_str());
        if(!db_param.empty()) m_db_param = db_param;
        m_db = DBHandler::createDBHandler(string("leveldb"), m_db_param, m_logger);
    }
