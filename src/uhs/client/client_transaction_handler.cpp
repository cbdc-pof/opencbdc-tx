...
namespace cbdc {
    ...
    std::future<bool> client_transaction_handler::execute_txn(
        ...
        m_logger->trace("Executing a txn ...");
        ...
        auto res_cb
            = [promise, &wallet, txn, this](
                  ...
                  m_logger->trace("Received a Response ...");
                  if(!res.has_value()) {
    ...
    client_transaction_handler::receive_transaction(
        cbdc::transaction::swap_wallet& wallet,
        const cbdc::transaction::input& prev_input,
        cbdc::skey_t skey,
        uint64_t expiry,
        cbdc::pubkey_t sender_addr,
        cbdc::pubkey_t payee) {
        auto txn = wallet.create_txn_tnswap_receive(prev_input,
                                                    expiry,
                                                    sender_addr,
                                                    payee,
                                                    skey);
        if(txn) {
            // Making a synchronoous call here
            auto result = execute_txn(wallet, txn.value()).get();
            if(result) {
                return txn;
            }
    ...
    std::optional<cbdc::transaction::full_tx>
    client_transaction_handler::refund_transaction(
        cbdc::transaction::swap_wallet& wallet,
