...
static int run_tier_nolan_swap(cbdc::config::options& cfg,
                        std::shared_ptr<cbdc::logging::log> logger, cbdc::sentinel::rpc::client &sentinel_client, cbdc::coordinator::rpc::client &coordinator_client) {
    ...
    mint_money(alice_wallet, mint_count, mint_amount, cfg, coordinator_client, logger);
    ...
    logger->info("secret: ", cbdc::to_string(secret_key), "\n secret_key_hash: ", cbdc::to_string(secret_key_hash));
    const uint64_t expiry_time = alice_wallet.set_expiry(10); // 10 minutes expiry

    logger->info("Creating Tier Nolan Swap transaction...");
    auto initial_txn = txn_handler.create_transaction(
        alice_wallet, mint_amount, bob_pubkey, secret_key_hash, expiry_time);
    ...
    auto ins = cbdc::transaction::swap_wallet::export_raw_inputs(initial_txn.value());
    ...
    auto received_txn = txn_handler.receive_transaction(
        bob_wallet, ins[0], secret_key, expiry_time,
...
int main(int argc, char** argv) {
    ...
    cbdc::sentinel::rpc::client sentinel_client(cfg.m_sentinel_endpoints, logger);
    ...
    return run_tier_nolan_swap(cfg, logger, sentinel_client, coordinator_client);
}
