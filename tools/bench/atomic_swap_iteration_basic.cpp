...
int main(int argc, char** argv) {
    ...
    using alice_wallet_t = cbdc::transaction::swap_wallet;
    using bob_wallet_t = cbdc::transaction::swap_wallet;



    // Wallet Initialization
    alice_wallet_t alice_wallet{};
    bob_wallet_t bob_wallet{};
    ...
    mint_money(alice_wallet, 5, 1000, cfg, coordinator_client, logger);
    ...
    logger->trace("Alice Balance CBDC1: ", alice_wallet.balance());
    logger->trace("Bob Balance CBDC1: ", bob_wallet.balance());
    

    logger->trace("STEP1 -- Initializing data");
    const auto amount = 1000;
    logger->trace("Swap Amount Decided is:",
                  amount);

    const auto bob_pubk = bob_wallet.generate_key();

    const std::string hex_sk = "96f5dd802dfe4c171796788788540adee26ca16b89811b7952daffdc2d190b3c";
    const std::string hex_sk_hash = "70bd21c6a0e7495ec72dc660915b93a1bafa5ed327ee538184d16ad4ef3ded19";
     
    const cbdc::skey_t sk = cbdc::hash_from_hex(hex_sk);
    const cbdc::skey_hash_t sk_hash = cbdc::hash_from_hex(hex_sk_hash);
    

    // Set the expiry time for the transaction
    const uint64_t expiry_time = alice_wallet.set_expiry(0);

    logger->trace("Initial Transaction on CBDC1 ");
    // Create the initial transaction for Alice to send to Bob
    
    auto intiator_txn = creator(alice_wallet, amount, bob_pubk, sk_hash, expiry_time);
    auto intiator_sender_key = alice_wallet.get_pubkey_swap_session(sk_hash);

    if (!intiator_txn.has_value()) {
        logger->error("Joiner Inititor txn Failed");
        return -1;
    } else {
        logger->trace("Alice has initiated the txn successfully ");
    }

    static std::atomic_bool running{true}, pause{false};
    ...
    auto joiner_spend_txn = receiver(bob_wallet, intiator_txn.value(), sk, expiry_time, intiator_sender_key.value(), bob_pubk);
    if (!joiner_spend_txn.has_value()) {
        logger->error("Spending Joiner txn failed");
        return -1;
    } else {
        logger->trace("Bob has successfully spent the transaction created by Alice");
    }
