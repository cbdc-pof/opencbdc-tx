...
class TnSwapWitnessValidationTest  : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        m_valid_create_receive_tx = wallet2.create_txn_tnswap_receive(cbdc::transaction::swap_wallet::export_raw_inputs(m_valid_create_tx)[0], m_expiry_time, wallet1.get_pubkey_swap_session(m_skey_hash).value(), 
...
TEST_F(TnSwapWitnessValidationTest, UnpackValidRefundWitness) {
    auto idx = 0; // Assume valid witness index
     auto txn  = wallet1.create_txn_tnswap_refund(cbdc::transaction::swap_wallet::export_raw_inputs(m_valid_create_tx)[0], m_expiry_time, wallet1.get_pubkey_swap_session(m_skey_hash).value(), 
